...
class AppTest {
    ...
    private fun testInputs(xmlInputs: String, threads: Int) {
        ...
        // --threads 1 --inputs simple.xml --output simple.actual.txt
        main(args)
    ...
    private fun assertFilesHaveSameContent(expectedFileName: String, actualFileName: String, message: String? = null) {
        val actual = Paths.get(TEMPORARY_DIRECTORY).resolve(actualFileName).toFile().readText().replace("\r", "").replace("\n", "")
        val expected = Paths.get(TEST_DATA_PATH).resolve(expectedFileName).toFile().readText().replace("\r", "").replace("\n", "")
